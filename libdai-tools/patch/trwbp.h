...
class TRWBP : public BP {
    ...
    //@{
        ...
        /// Optimize over spanning tree polytope
        void updateEdgeWeights( Real alpha,std::string update );

		/// Run inference
		Real runInference(size_t linesearch=0);

		void lineSearch(const RootedTree &mst, const std::vector<Real> &gradient);
    protected:
