...
//int main(int argc,char* argv[]){ //char* uaifname) {
int readUAI(GraphicalModelType& gm,std::string uaifile){
   ...
   if(myuaifile.eof()) {
      ...
      for(IndexType f=0; f<numFactors; ++f){
         ...
         // add function to gm
         GraphicalModelType::FunctionIdentifier currentFunctionIndex = gm.addSharedFunction(currentFunction);
